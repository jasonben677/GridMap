...
namespace toinfiniityandbeyond.Tilemapping
{
    partial class TileMapEditor : Editor
    {
        ...
        {
            this.tileMap.ChangeWorkArea(this.tileMap.workArea);

            if (this.tileMap.mapStruct == null)
            {
                this.tileMap.SetMapStruct();
                this.tileMap.SetMapdata();
                this.tileMap.mapStruct.OnLoad_NewMapdata();
                this.tileMap.mapStruct.CreateGrid(true);
            }
        ...
        private void OnSceneGUI()
        {
            if (tileMap.isInEditMode)
            {
                tileMap.toolbarWindowPosition = ClampToScreen(GUILayout.Window(GUIUtility.GetControlID(FocusType.Passive), tileMap.toolbarWindowPosition,(int id) => { this._toolbarId = id; ToolbarWindow(id); }, new GUIContent("Toolbar"), GUILayout.Width(80)));
        ...
        private void ToolbarWindow(int id)
        {
            ...
            if (tileMap.secondaryTile)
            {
                ...
                tileName = (text.Length > 9) ? $"{text.Substring(0, 9)}..." : text;
            }
            GUI.Label(secondaryTileRect, new GUIContent(tileName, text), MyStyles.centerWhiteMiniLabel);
            ...
            GUI.Label(primaryTileRect, new GUIContent(tileName, text), MyStyles.centerWhiteBoldLabel);
            GUI.contentColor = Color.white;
            ...
            if (tileMap.scriptableTileCache.Count > 0)
            {
                ...
                for (int y = 0; y < rows; y++)
                {
                    ...
                    for (int x = 0; x < columns; x++)
                    {
                        ...
                        string tileName = (tileText.Length > 5) ? $"{tileText.Substring(0, 5)}..." : tileText;
                        GUI.Label(buttonRect, new GUIContent(tileName, tileText), MyStyles.tileTextBoldLabel);
                    }
                    GUILayout.EndHorizontal();
                }
        ...
        {
            float camW = MapEditorDefine.ChangeToEditorSize(this.tileMap.marginX);
            float camH = MapEditorDefine.ChangeToEditorSize(this.tileMap.marginY);

            float width = MapEditorDefine.ChangeToEditorSize(this.tileMap.mapWidth) + (camW * 2);
            float height = MapEditorDefine.ChangeToEditorSize(this.tileMap.mapHeight) + (camH * 2);

            this.tileMap.transform.localPosition = new Vector3(-camW, -camH, 0);

            Vector3 position = new Vector3(-camW, -camH);
        ...
        private void HandleMouseEvents()
        {
            ...
            if (tileMap.selectedScriptableTool >= 0 && tileMap.selectedScriptableTool < tileMap.scriptableToolCache.Count)
            {
                if (e.button == 0)
                {
                    this.GetMousePosition(ref point);

                    this.tileMap.mapStruct.nodeArr.TryGetValue(point.x + "_" + point.y, out RoadNode roadNode);
                    ...
                    ScriptableTool selectTool = tileMap.scriptableToolCache[tileMap.selectedScriptableTool];

                    selectTool.GetRegion(point, this.tileMap.primaryTile, this.tileMap);

                    GUIStyle style = new GUIStyle(MyStyles.centerWhiteBoldLabel);
                    style.normal.textColor = Color.green;

                    if (roadNode != null) Handles.Label(new Vector2(roadNode.cx, roadNode.cy), point.ToString(), style);
                }
            }
        ...
        private bool GetMousePosition(ref Point point)
        {
            ...
            {
                Vector3 ray_point = (ray.origin + (ray.direction.normalized * distance) - position);
                Vector2 transform_point = this.tileMap.mapStruct.GetClick(ray_point.x, ray_point.y);
                point = new Point(transform_point);
            }

            bool result = (point.x >= 0 && point.x < tileMap.countX && point.y >= 0 && point.y < tileMap.countY);
        ...
        private void _BringToolbarToFront()
        {
            GUI.BringWindowToFront(this._toolbarId);
        }
